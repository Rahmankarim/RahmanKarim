...
const rahman = {
  ...
  founder: "Indeset Pvt Ltd (Feb 2026 – present)",
  aiIntern: "FlyRank AI — machine learning projects",
...
MERN stack with Stripe Connect, Prisma atomic transactions, and JWT auth. Dual-license model (exclusive / non-exclusive), 5% platform commission with automated 95% payouts to designers via Stripe Connect. Includes a race-condition fix implemented through atomic DB transactions.

`MongoDB` `Express` `React` `Node` `Stripe` `Prisma`
...
Final Year Project — Next.js, MongoDB, Socket.io, Groq API. Companion matching via a TravelIntent schema, real-time chat, guide badge tiers, gamified leaderboard, and role-based access for Travelers, Guides, and Admins.

`Next.js` `Socket.io` `Groq API` `MongoDB`
...
Full-stack Next.js 14 app scaffolded with Mongoose models, NextAuth, and Claude AI integration for automated, intelligent grading workflows.

`Next.js 14` `Mongoose` `NextAuth` `Claude API`
...
Currently shipping products as Founder & CEO of Indeset, alongside ML work as an AI Intern at FlyRank AI.

`AI Integrations` `Product Building`
